...
class FindingsHelper : ScrollablePanel() {
  ...
  private fun updateHelp() {
    ...
    if (this.sast != null) {
      val title = sast?.title.toString().lowercase()?.replaceFirstChar { it.uppercase() } ?: "SAST Finding"
      val confidence = sast?.confidence?.toString()?.lowercase()?.replaceFirstChar { it.uppercase() }
        sast?.impact?.toString()?.lowercase()?.replaceFirstChar { it.uppercase() }
      val likelihood = sast?.likelihood?.toString()?.lowercase()?.replaceFirstChar { it.uppercase() }
