...
internal class TrivyBackgroundRunTask(
  ...
  override fun run() {
    ...
    // Read the freshest `useAquaPlatform` value from project settings at the point
    // we configure the command. Avoid creating a per-run message-bus connection
    // which would otherwise be created for every run.
    ...
    val projSettingsForPaths = TrivyProjectSettingState.getInstance(project)
    if (projSettingsForPaths.useConfig && projSettingsForPaths.configPath != "") {
      commandParts.add(String.format("--config=%s", projSettingsForPaths.configPath))
    }

    if (projSettingsForPaths.useIgnore && projSettingsForPaths.ignorePath != "") {
      commandParts.add(String.format("--ignorefile=%s", projSettingsForPaths.ignorePath))
    }
    ...
    if (!projSettingsForPaths.skipDirList.isEmpty()) {
      // combine the skip directories into a single comma separated string
      val skipDirs = projSettingsForPaths.skipDirList.joinToString(separator = ",")
    ...
    // Use the freshest value for Aqua Platform at the point we configure the command
    if (TrivyProjectSettingState.getInstance(project).useAquaPlatform) {
      super.setTitle("Running Aqua Platform Scan")
      // re-fetch current project settings to apply feature flags and config
      val currentProjectSettings = TrivyProjectSettingState.getInstance(project)
      configureCommandLineEnv(commandLine, currentProjectSettings, resultFile)
      // verify the credentials first using the env we just configured
      if (
    ...
    try {
      // Capture output (stdout+stderr) and wait for the process to finish so we can inspect
      // exit code / panic
      val output =
      ...
      val panicked =
        output.lineSequence().any { it.trimStart().startsWith("panic:", ignoreCase = true) }
